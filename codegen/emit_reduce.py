...
def _sym(buf: str, idx: tuple) -> str:
    return f"v_{buf}" + "".join(f"_{i}" for i in idx)
...
def emit_reduce_source(prog: Program, sched: Schedule, block: int = 128,
    ...
    for buf in spec.live_out:
        ...
        for idx in sorted(all_indices(t.sizes)):
            ...
            if scatter is not None:
                # Several segment elements target the same output row, so the store accumulates.
                # `cute.arch.atomic_add` takes a pointer, so the flat offset is computed from the
                # buffer's row-major layout: scatter target row, plus this element's offset
                # within the row.
                coords = ", ".join(["m_" + scatter + "[e]"] + [str(i) for i in idx])
                body.append(f"cute.arch.atomic_add(m_{buf}.iterator + "
                            f"m_{buf}.layout(({coords})), {value})")
            else:
                body.append(f"{_ref(prog, buf, idx)} = {value}")
